...
<div class="px-6 pt-8 pb-4">
    ...
    <p class="text-sm text-white/50">Temukan tools untuk membangun skill data kamu</p>
</div>
    ...
    <form method="GET" action="{{ route('katalog') }}" class="flex gap-3 w-full">
        <div class="relative flex-1">
            ...
            <input type="text" name="search" value="{{ request('search') }}"
                placeholder="Cari sandbox tools..."
        ...
        <select name="sort" onchange="this.form.submit()"
            ...
            <option value="token" {{ request('sort')=='token'?'selected':'' }}>Token/menit</option>
        </select>
...
<div class="px-6">
    <main class="flex-1 min-w-0">
        <p class="text-xs text-white/35 mb-4">
            Menampilkan <span class="text-green-400">{{ $tools->total() }}</span> tools
        </p>
        ...
        <div class="grid grid-cols-3 gap-3 mb-6">
            ...
            <a href="{{ $tool->url ?? '#' }}"
               ...
               class="card block bg-gray-900 border border-white/10 rounded-xl p-4 hover:border-green-400/40 transition-colors">
                ...
                {{-- Icon --}}
                <div class="w-9 h-9 rounded-lg flex items-center justify-center text-xs font-medium mb-3 text-white bg-indigo-900">
                    {{ strtoupper(substr($tool->nama_sandbox, 0, 2)) }}
                </div>

                {{-- Nama --}}
                <h3 class="text-sm font-medium text-white leading-tight mb-2">
                    {{ $tool->nama_sandbox }}
                </h3>

                {{-- Deskripsi Singkat --}}
                <p class="text-xs text-white/45 leading-relaxed mb-3 line-clamp-2">
                    {{ $tool->deskripsi_singkat ?? 'Tidak ada deskripsi.' }}
                </p>

                {{-- Akses Perangkat --}}
                @if($tool->akses_perangkat)
                <div class="flex flex-wrap gap-1 mb-3">
                    @foreach(explode(',', $tool->akses_perangkat) as $akses)
                    <span class="text-xs px-1.5 py-0.5 bg-white/5 text-white/40 rounded">
                        {{ trim($akses) }}
                    </span>
                    @endforeach
                </div>
                ...
                <div class="flex items-center justify-between pt-2 border-t border-white/10">
                    <span class="text-xs text-white/30">
                        {{ $tool->token_per_menit ?? '-' }} token/menit
                    </span>
                    @if($tool->url)
                    <span class="text-xs text-green-400">Buka &#8599;</span>
                    @endif
                </div>
            </a>
            @endforeach
