    ...
    <div class="max-w-full px-6 py-2.5 flex items-center justify-between">
        ...
        <div class="flex items-center gap-6">
            ...
            <div class="hidden md:flex items-center gap-1">
                <a href="/"
                   class="px-3 py-1.5 text-sm rounded-full transition-colors {{ request()->is('/') ? 'bg-gray-900 text-white font-semibold' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-100' }}">
                    Home
                </a>

                <a href="{{ route('learn') }}"
                   class="px-3 py-1.5 text-sm rounded-full font-semibold transition-colors {{ request()->routeIs('learn') || request()->routeIs('dashboard') ? 'bg-gray-900 text-white' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-100' }}">
                    Learn
                </a>

                <a href="{{ route('ai-native') }}"
                   class="px-3 py-1.5 text-sm rounded-full flex items-center gap-1 transition-colors {{ request()->routeIs('ai-native') ? 'bg-purple-100 text-purple-700' : 'text-purple-600 hover:text-purple-700 hover:bg-purple-50' }}">
                    <span>✦</span> AI Native
                </a>

                <a href="{{ route('certification.index') }}"
                   class="px-3 py-1.5 text-sm rounded-full transition-colors {{ request()->routeIs('certification*') ? 'bg-gray-900 text-white font-semibold' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-100' }}">
                    Certification
                </a>

                {{-- FIX: Sandbox sekarang pakai route('practice') dengan styling yang benar --}}
                <a href="{{ route('sandbox') }}"
                   class="{{ request()->routeIs('sandbox') ? 'bg-gray-900 text-white font-semibold' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-100' }}">
                </a>
            </div>
        </div>
        ...
        <div class="flex items-center gap-2">
            ...
            <div class="relative hidden md:block">
                ...
                <input type="text" placeholder="Search"
                       class="bg-gray-100 border-none text-sm pl-9 pr-4 py-1.5 rounded-lg w-44 focus:outline-none focus:ring-2 focus:ring-green-400 focus:w-56 transition-all">
            </div>
            ...
            {{-- Upgrade --}}
            <a href="{{ route('harga') }}"
               class="bg-purple-600 text-white text-xs font-medium px-3 py-1.5 rounded-lg hover:bg-purple-700 transition-colors">
                Upgrade
            </a>
            ...
            <span class="text-gray-500 text-sm flex items-center gap-1 cursor-pointer">🌐 EN</span>
            <a href="{{ route('login') }}"
               class="text-sm text-gray-700 font-medium px-3 py-1.5 border border-gray-300 rounded-lg hover:border-gray-400">
                Log In
            </a>
