...
<div class="flex min-h-screen">
    ...
    <main class="flex-1 p-6 max-w-5xl">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <a href="{{ route('course.learn', $enrollment->course->slug ?? '#') }}"
                               class="text-base font-semibold text-gray-900 hover:text-green-600 flex items-center gap-1">
                                {{ $enrollment->course->title ?? $enrollment->course->nama_course ?? 'Course' }} →
                            </a>
        ...
        <div class="grid grid-cols-3 gap-4 mb-6">
            ...
            {{-- ASSESS --}}
            <a href="{{ route('assessments') }}" class="card p-4 flex items-center gap-3 hover:shadow-md transition-shadow">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-sm font-semibold text-gray-900 truncate">Analytic Fundamentals</p>
                </div>
                <svg class="shrink-0" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#999" stroke-width="2"><path d="M9 18l6-6-6-6"/></svg>
            </a>
            ...
            <a href="{{ route('practice.index') }}" class="card p-4 flex items-center gap-3 hover:shadow-md transition-shadow">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-sm font-semibold text-gray-900 truncate">Introduction to Python</p>
                </div>
                <svg class="shrink-0" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#999" stroke-width="2"><path d="M9 18l6-6-6-6"/></svg>
            </a>

            {{-- APPLY --}}
            <a href="{{ route('real-world-projects') }}" class="card p-4 flex items-center gap-3 hover:shadow-md transition-shadow">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-sm font-semibold text-gray-900 truncate">Real World Project</p>
                </div>
                <svg class="shrink-0" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#999" stroke-width="2"><path d="M9 18l6-6-6-6"/></svg>
            </a>
        ...
        <div class="grid grid-cols-3 gap-4">
            ...
            {{-- ✅ Leaderboard pakai Stored Procedure GetLeaderboard(5) --}}
            <div class="card p-5">
                <div class="flex items-center justify-between mb-4">
                    <a href="{{ route('leaderboard') }}" class="text-sm font-semibold text-gray-900 hover:text-green-600">Leaderboard →</a>
                ...
                @php
                    try {
                        ...
                        // Fallback kalau stored procedure belum ada
                        $topUsers = \App\Models\User::orderByDesc('xp')->take(5)->get()->toArray();
                        $topUsers = array_map(fn($u) => (object)$u, $topUsers);
                    }
                @endphp

                @foreach($topUsers as $i => $u)
                <div class="flex items-center gap-3 py-2 {{ $i < count($topUsers)-1 ? 'border-b border-gray-100' : '' }}">
            ...
            <div class="col-span-2">
                ...
                <div class="space-y-3">
                    @php
                        $featured = \App\Models\Course::where('is_featured', true)->take(3)->get();
                        // Fallback kalau tidak ada yang is_featured
                        if ($featured->isEmpty()) {
                            $featured = \App\Models\Course::latest()->take(3)->get();
                        }
                    @endphp
                    ...
                    <a href="{{ route('course.detail', $course->slug) }}"
                       class="card p-4 flex items-center gap-4 hover:shadow-md transition-shadow block">
                        <div class="w-10 h-10 rounded-lg flex items-center justify-center text-xs font-bold text-white shrink-0"
                            ...
                            {{ $course->icon_text ?? substr($course->title ?? $course->nama_course ?? '?', 0, 2) }}
                        </div>
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-medium text-gray-900 truncate">
                                {{ $course->title ?? $course->nama_course ?? '' }}
                            </p>
                            <p class="text-xs text-gray-500">
                                {{ $course->difficulty ?? 'Pemula' }} · {{ $course->duration_hours ?? 0 }}h · ★ {{ $course->rating ?? 0 }}
                            </p>
                        </div>
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#999" stroke-width="2"><path d="M9 18l6-6-6-6"/></svg>
                    </a>
                    @empty
                    <div class="card p-4 text-sm text-gray-500 text-center">
                        Belum ada kursus tersedia. <a href="{{ route('courses') }}" class="text-green-600">Browse semua →</a>
                    </div>
                    @endforelse
                </div>
            </div>
